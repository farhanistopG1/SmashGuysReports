...
def daily_metrics(bills: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    dine_in = [bill for bill in bills if bill not in ta and bill not in staff]
    apc = (gross * (Decimal("1") - APC_DEDUCTION) / covers) if covers else Decimal("0")
    return {
        "bills": len(bills), "covers": covers, "gross": gross, "apc": apc, "collections": collections,
        "dine_in": (sum((bill_total(bill) for bill in dine_in), Decimal("0")), len(dine_in)),
...
def render(day: date, daily: dict[str, Any], menu: dict[str, int], removed: tuple[int, Decimal]) -> str:
    dine_amount, dine_bills = daily["dine_in"]
...
Dine in : {display_money(dine_amount)}/{dine_bills}
